

translate korean strings:


    old "01. Worst Nightmare"
    new "01. Worst Nightmare"


    old "Unlock the hidden 19th ending"
    new "Unlock the hidden 19th ending"


    old "02. GG EZ Videogame"
    new "02. GG EZ Videogame"


    old "Get a high score in the minigame"
    new "Get a high score in the minigame"


    old "03. Somebody that I Used to Know"
    new "03. Somebody that I Used to Know"


    old "Unlock Lydia's music video"
    new "Unlock Lydia's music video"


    old "04. Full Course"
    new "04. Full Course"


    old "Unlock 18 endings"
    new "Unlock 18 endings"


    old "05. Front Nine"
    new "05. Front Nine"


    old "Unlock 9 endings"
    new "Unlock 9 endings"


    old "06. What a Wild Ride"
    new "06. What a Wild Ride"


    old "Complete all 6 base endings"
    new "Complete all 6 base endings"


    old "07. WTF Bro?"
    new "07. WTF Bro?"


    old "See everything you never wanted to"
    new "See everything you never wanted to"


    old "08. Hacker"
    new "08. Hacker"


    old "View all the secret door codes"
    new "View all the secret door codes"


    old "09. In the Beginning..."
    new "09. In the Beginning..."


    old "Make it all the way to Day 15"
    new "Make it all the way to Day 15"


    old "10. Release the Kraken"
    new "10. Release the Kraken"


    old "Get free from the cage"
    new "Get free from the cage"


    old "11. Sudo Admin"
    new "11. Sudo Admin"


    old "Enter the server room"
    new "Enter the server room"


    old "12. rm -rf"
    new "12. rm -rf"


    old "Destroy the app... or not"
    new "Destroy the app... or not"


    old "13. The People Versus..."
    new "13. The People Versus..."


    old "Go to the court trial"
    new "Go to the court trial"


    old "14. Cherry Popped"
    new "14. Cherry Popped"


    old "Finish your first time"
    new "Finish your first time"


    old "15. Miss White, with the Candlestick"
    new "15. Miss White, with the Candlestick"


    old "Discover who is behind the app on the LC path"
    new "Discover who is behind the app on the LC path"


    old "16. Keyser Soze"
    new "16. Keyser Soze"


    old "Discover who is behind the app on the NTR path"
    new "Discover who is behind the app on the NTR path"


    old "17. Real Genius"
    new "17. Real Genius"


    old "Finish final exams perfectly"
    new "Finish final exams perfectly"


    old "18. Keep Pounding That Gavel"
    new "18. Keep Pounding That Gavel"


    old "Get both scenes with the judge"
    new "Get both scenes with the judge"


    old "19. What Are Friends For"
    new "19. What Are Friends For"


    old "Knock up your best friend's gal"
    new "Knock up your best friend's gal"


    old "20. Are You Not Entertained"
    new "20. Are You Not Entertained"


    old "Play all three scenes at the amusement park"
    new "Play all three scenes at the amusement park"


    old "21. Oh, the Humanity!"
    new "21. Oh, the Humanity!"


    old "Play all three versions of Lyssa's Wednesday scenes"
    new "Play all three versions of Lyssa's Wednesday scenes"


    old "22. Why Would You Do That"
    new "22. Why Would You Do That"


    old "Don't remove the cage, even though you could"
    new "Don't remove the cage, even though you could"


    old "23. Fantastic Fivesome"
    new "23. Fantastic Fivesome"


    old "Maria and some friends are very thankful"
    new "Maria and some friends are very thankful"


    old "24. Craving Pickles and Ice-cream"
    new "24. Craving Pickles and Ice-cream"


    old "Impregnate every possible person in the queen's harem"
    new "Impregnate every possible person in the queen's harem"


    old "25. I Need Sexual Healing"
    new "25. I Need Sexual Healing"


    old "Helped all 3 couples at the ski resort"
    new "Helped all 3 couples at the ski resort"


    old "26. This Magic Moment"
    new "26. This Magic Moment"


    old "Went on the ferris wheel with Daisy"
    new "Went on the ferris wheel with Daisy"


    old "27. May I panic now, Mistress?"
    new "27. May I panic now, Mistress?"


    old "Braved the funhouse with Dahlia"
    new "Braved the funhouse with Dahlia"


    old "28. Fifty-Five Foot Club"
    new "28. Fifty-Five Foot Club"


    old "Join Polly and Nora on the ferris wheel"
    new "Join Polly and Nora on the ferris wheel"


    old "29. That Was Cathartic... for Them"
    new "29. That Was Cathartic... for Them"


    old "Fix Dahlia and Samiya's relationship"
    new "Fix Dahlia and Samiya's relationship"


    old "HIGH SCORE :\n[persistent.minigame_max_score]"
    new "HIGH SCORE :\n[persistent.minigame_max_score]"


    old "HOW MANY ENDINGS HAVE YOU UNLOCKED?"
    new "HOW MANY ENDINGS HAVE YOU UNLOCKED?"


    old "9 ENDINGS"
    new "9 ENDINGS"


    old "YOU GOTTA PUMP THOSE NUMBERS UP.\nTHOSE ARE ROOKIE NUMBERS!"
    new "YOU GOTTA PUMP THOSE NUMBERS UP.\nTHOSE ARE ROOKIE NUMBERS!"


    old "6 endings\naren't enough.\nI need all 18 now!!!"
    new "6 endings\naren't enough.\nI need all 18 now!!!"


    old "{b}CHANGE MY MIND{/b}"
    new "{b}CHANGE MY MIND{/b}"


    old "I WANT TO KEEP\nFETISH LOCATOR"
    new "I WANT TO KEEP\nFETISH LOCATOR"


    old "TO MAKE SURE NO ONE\nWILL BE BLACKMAILED"
    new "TO MAKE SURE NO ONE\nWILL BE BLACKMAILED"


    old "YOU'RE GOING TO USE\nIT FOR GOOD, RIGHT?"
    new "YOU'RE GOING TO USE\nIT FOR GOOD, RIGHT?"


    old "RIGHT?"
    new "RIGHT?"


    old "THE FIRST ENDING\nFROM 18 TOTAL ENDINGS"
    new "THE FIRST ENDING\nFROM 18 TOTAL ENDINGS"


    old "IS THAT A REAL ENDING?"
    new "IS THAT A REAL ENDING?"


    old "TRADE OFFER"
    new "TRADE OFFER"


    old "I receive:"
    new "I receive:"


    old "Cum in every pussy\nat the wedding"
    new "Cum in every pussy\nat the wedding"


    old "You receive:"
    new "You receive:"


    old "The whole\nharem pregnant"
    new "The whole\nharem pregnant"


    old "30. What? You wanted more?"
    new "30. What? You wanted more?"


    old "Finish all 6 Endings from DLC-1"
    new "Finish all 6 Endings from DLC-1"


    old "31. Go Debbie Go"
    new "31. Go Debbie Go"


    old "Have sex with Debbie"
    new "Have sex with Debbie"


    old "32. Fully Cooked"
    new "32. Fully Cooked"


    old "Get max points in Lydia's challenges in e07"
    new "Get max points in Lydia's challenges in e07"


    old "33. Full House"
    new "33. Full House"


    old "Play with all the characters in e04"
    new "Play with all the characters in e04"


    old "34. Day and Night"
    new "34. Day and Night"


    old "Finish e04 and e07"
    new "Finish e04 and e07"


    old "35. A lucky dozen"
    new "35. A lucky dozen"


    old "Finish 12 endings"
    new "Finish 12 endings"


    old "ORNY"
    new "ORNY"


    old "H"
    new "H"


    old "UNGRY"
    new "UNGRY"


    old "APPY"
    new "APPY"

  # Decompiled by unrpyc_v1.2.0-alpha: https://github.com/CensoredUsername/unrpyc
