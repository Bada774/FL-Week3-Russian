
translate turkish lovense_test_9e0ee773:


    "Test 1 (2)"


translate turkish lovense_test_6cca6db3:


    "Test 2 (20)"


translate turkish lovense_test_fa719a86:


    "Titreşim Testi 0"


translate turkish lovense_test_d7e7b816:


    "Pompa Testi DÜŞÜK"


translate turkish lovense_test_14a8f7e0:


    "Pompa Testi MAKS"


translate turkish lovense_test_30b42ad1:


    "Kalıp Testi"


translate turkish lovense_test_37bd1745:


    "Titreşim + Dönme testi"


translate turkish lovense_test_8ab253a3:


    "Testi durdur"

translate turkish strings:


    old "Please make sure both FL and Lovense Remote app are on the same LAN."
    new "Lütfen hem FL hem de Lovense Remote uygulamasının aynı LAN üzerinde olduğundan emin olun."


    old "Follow the steps to enable Game Mode in Lovense Remote app and connect your toy to FL."
    new "Lovense Remote uygulamasında Oyun Modunu etkinleştirmek için adımları izleyin ve oyuncağınızı FL'ye bağlayın."


    old "1. Click on the \"Me\" tab"
    new "1.\"Ben\" sekmesine tıklayın"


    old "2. Then click on \"Settings\""
    new "2. Ardından \"Ayarlar\" üzerine tıklayın"


    old "3. Click the button to enable \"Game Mode\""
    new "3. \"Oyun Modunu\" etkinleştirmek için düğmeyi tıklayın"


    old "4. Enter the \"Local IP\" and \"Http Port\" shown in the app"
    new "4. Uygulamada gösterilen \"Yerel IP\" ve \"Http Port\" girin"


    old "Local IP: "
    new "Yerel IP: "


    old "{}"
    new "{}"


    old "HTTP Port: "
    new "HTTP Bağlantı Noktası: "


    old "Connect"
    new "Bağlan"


    old "Toy"
    new "Oyuncak"


    old "Your favorite toy now works with Fetish Locator!"
    new "En sevdiğiniz oyuncağınız artık Fetish Locator ile çalışıyor!"


    old "Get any Lovense toy with our exclusive 55% discount and sync it with Fetish Locator!"
    new "Herhangi bir Lovense oyuncağını özel %55 indirimimizle alın ve Fetish Locator ile senkronize edin!"


    old "Limited time offer!"
    new "Sınırlı süreli teklif!"


    old "Get your discount now"
    new "İndiriminizi şimdi alın"


    old "{u}Connect your toy!{/u}"
    new "{u}Oyuncağınızı bağlayın!{/u}"


    old "Please enter your Local IP number:"
    new "Lütfen Yerel IP numaranızı girin:"


    old "Please enter your HTTP Port number:"
    new "Lütfen HTTP Bağlantı Noktası numaranızı girin:"


    old "Back"
    new "Geri"


    old "Save"
    new "Kaydet"

translate turkish strings:


    old "Get a Lovense toy and enjoy the best way to experience Fetish Locator!"
    new "Bir Lovense oyuncağı alın ve Fetish Locator'u deneyimlemenin en iyi yolunun tadını çıkarın!"


    old "Get a toy now"
    new "Hemen bir oyuncak al"

translate turkish strings:


    old "1. Click on the \"Discover\" tab"
    new "1. \\“Keşfet\\” sekmesine tıklayın"


    old "2. Then click on \"Game Mode\" option"
    new "2. Ardından \\“Oyun Modu\\” seçeneğine tıklayın"


    old "4. Enter the \"Local IP\" and \"Port\" shown in the app\nThen click Connect."
    new "4. Uygulamada gösterilen \\“Yerel IP\\” ve \\“Bağlantı Noktası\\” bilgilerini girin\nArdından Bağlan'a tıklayın."
# Decompiled by unrpyc: https://github.com/CensoredUsername/unrpyc
