

translate turkish strings:


    old "01. Worst Nightmare"
    new "01. En Kötü Kabus"


    old "Unlock the hidden 19th ending"
    new "Gizli 19. sonun kilidini aç"


    old "02. GG EZ Videogame"
    new "02. GG EZ Video Oyunu"


    old "Get a high score in the minigame"
    new "Mini oyunda yüksek puan al"


    old "03. Somebody that I Used to Know"
    new "03. Eskiden Tanıdığım Biri"


    old "Unlock Lydia's music video"
    new "Lydia'nın müzik videosunun kilidini aç "


    old "04. Full Course"
    new "04. Tam Kurs"


    old "Unlock 18 endings"
    new "18 sonun kilidini aç"


    old "05. Front Nine"
    new "05. Ön Dokuz"


    old "Unlock 9 endings"
    new "9 sonun kilidini aç"


    old "06. What a Wild Ride"
    new "06. Ne Vahşi Bir Yolculuk"


    old "Complete all 6 base endings"
    new "6 temel sonun hepsini tamamla"


    old "07. WTF Bro?"
    new "07. WTF Kardeşim?"


    old "See everything you never wanted to"
    new "Asla görmek istemediğiniz her şeyi görün"


    old "08. Hacker"
    new "08. Hacker"


    old "View all the secret door codes"
    new "Tüm gizli kapı kodlarını görüntüle"


    old "09. In the Beginning..."
    new "09. Başlangıçta..."


    old "Make it all the way to Day 15"
    new "15. Güne kadar devam edin"


    old "10. Release the Kraken"
    new "10. Kraken'i Serbest Bırakın"


    old "Get free from the cage"
    new "Kafesten kurtulun"


    old "11. Sudo Admin"
    new "11. Sudo Yöneticisi"


    old "Enter the server room"
    new "Sunucu odasına girin"


    old "12. rm -rf"
    new "12. rm -rf"


    old "Destroy the app... or not"
    new "Uygulamayı yok et... veya etme"


    old "13. The People Versus..."
    new "13. İnsanlara Karşı..."


    old "Go to the court trial"
    new "Mahkemeye git duruşma"


    old "14. Cherry Popped"
    new "14. Kiraz Patlamış"


    old "Finish your first time"
    new "İlk seferinde bitir"


    old "15. Miss White, with the Candlestick"
    new "15. Şamdanlı Bayan Beyaz"


    old "Discover who is behind the app on the LC path"
    new "LC yolunda uygulamanın arkasında kimin olduğunu keşfedin"


    old "16. Keyser Soze"
    new "16. Keyser Söze"


    old "Discover who is behind the app on the NTR path"
    new "NTR yolunda uygulamanın arkasında kimin olduğunu keşfedin"


    old "17. Real Genius"
    new "17. Gerçek Dahi"


    old "Finish final exams perfectly"
    new "Final sınavlarını mükemmel bir şekilde bitirin"


    old "18. Keep Pounding That Gavel"
    new "18. Tokmağını Vurmaya Devam Et"


    old "Get both scenes with the judge"
    new "Yargıçla her iki sahneyi de alın"


    old "19. What Are Friends For"
    new "19. Arkadaşlar Ne İşe Yarar"


    old "Knock up your best friend's gal"
    new "En iyi arkadaşının kızını hamile bırak"


    old "20. Are You Not Entertained"
    new "20. Eğlenmedin mi"


    old "Play all three scenes at the amusement park"
    new "Eğlence parkındaki üç sahneyi de oynayın"


    old "21. Oh, the Humanity!"
    new "21. Ah, İnsanlık!"


    old "Play all three versions of Lyssa's Wednesday scenes"
    new "Lyssa'nın Çarşamba sahnelerinin üç versiyonunu da oynat"


    old "22. Why Would You Do That"
    new "22. Bunu Neden Yapasın"


    old "Don't remove the cage, even though you could"
    new "Yapabilseniz bile kafesi çıkarmayın"


    old "23. Fantastic Fivesome"
    new "23. Fantastik Beşli"


    old "Maria and some friends are very thankful"
    new "Maria ve bazı arkadaşlar çok müteşekkir"


    old "24. Craving Pickles and Ice-cream"
    new "24. Turşu ve Dondurma Aşermek"


    old "Impregnate every possible person in the queen's harem"
    new "Kraliçenin haremindeki herkesi hamile bırak"


    old "25. I Need Sexual Healing"
    new "25. Cinsel İyileşmeye İhtiyacım Var"


    old "Helped all 3 couples at the ski resort"
    new "Kayak merkezindeki 3 çifte de yardımcı oldu"


    old "26. This Magic Moment"
    new "26. Bu Sihirli An"


    old "Went on the ferris wheel with Daisy"
    new "Daisy ile dönme dolaba bindim"


    old "27. May I panic now, Mistress?"
    new "27. Şimdi paniğe kapılabilir miyim Hanımım?"


    old "Braved the funhouse with Dahlia"
    new "Dahlia ile lunaparka göğüs gerdi"


    old "28. Fifty-Five Foot Club"
    new "28. Elli Beş Ayak Kulübü"


    old "Join Polly and Nora on the ferris wheel"
    new "Dönme dolapta Polly ve Nora'ya katılın"


    old "29. That Was Cathartic... for Them"
    new "29. Bu Kathartikti... Onlar İçin"


    old "Fix Dahlia and Samiya's relationship"
    new "Dahlia ve Samiya'nın ilişkisini düzeltin"


    old "HIGH SCORE :\n[persistent.minigame_max_score]"
    new "YÜKSEK\nPUAN :\n[persistent.minigame_max_score]"


    old "HOW MANY ENDINGS HAVE YOU UNLOCKED?"
    new "KAÇ SONUN KİLİDİNİ AÇTINIZ?"


    old "9 ENDINGS"
    new "9 SON"


    old "YOU GOTTA PUMP THOSE NUMBERS UP.\nTHOSE ARE ROOKIE NUMBERS!"
    new "BU NUMARALARI YÜKSELTMEK ZORUNDASINIZ.\nBUNLAR ÇAYLAK NUMARALARI!"


    old "6 endings\naren't enough.\nI need all 18 now!!!"
    new "6 son yeterli değil.\nŞimdi 18 sonun\nhepsine\nihtiyacım var!!!"


    old "{b}CHANGE MY MIND{/b}"
    new "{b}FİKRİMİ DEĞİŞTİR{/b}"


    old "I WANT TO KEEP\nFETISH LOCATOR"
    new "TUTMAK İSTİYORUMP\nFETISH LOCATOR"


    old "TO MAKE SURE NO ONE\nWILL BE BLACKMAILED"
    new "KİMSENİN ŞANTAJ\nGÖRMEYECEĞİNDEN\nEMİN OLMAK İÇİN"


    old "YOU'RE GOING TO USE\nIT FOR GOOD, RIGHT?"
    new "BUNU İYİ İÇİN\nKULLANACAKSINIZ,\nDEĞİL Mİ?"


    old "RIGHT?"
    new "DOĞRU?"


    old "THE FIRST ENDING\nFROM 18 TOTAL ENDINGS"
    new "TOPLAM 18 SONA\nGÖRE İLK SON"


    old "IS THAT A REAL ENDING?"
    new "BU GERÇEK BİR SON MU?"


    old "TRADE OFFER"
    new "TİCARET TEKLİFİ"


    old "I receive:"
    new "Alıyorum:"


    old "Cum in every pussy\nat the wedding"
    new "Düğünde her\namcığa boşalmak"


    old "You receive:"
    new "Aldığınız:"


    old "The whole\nharem pregnant"
    new "Bütün\nharem hamile"


    old "30. What? You wanted more?"
    new "30. Ne? Daha fazlasını mı istedin?"


    old "Finish all 6 Endings from DLC-1"
    new "DLC-1'deki 6 Sonun hepsini bitir"


    old "31. Go Debbie Go"
    new "31. Git Debbie Git"


    old "Have sex with Debbie"
    new "Debbie ile seks yap"


    old "32. Fully Cooked"
    new "32. Tamamen Pişmiş"


    old "Get max points in Lydia's challenges in e07"
    new "e07'de Lydia'nın mücadelelerinde maksimum puan alın"


    old "33. Full House"
    new "33. Tam Ev"


    old "Play with all the characters in e04"
    new "e04'teki tüm karakterlerle oynayın"


    old "34. Day and Night"
    new "34. Gündüz ve Gece"


    old "Finish e04 and e07"
    new "e04 ve e07'yi bitirin"


    old "35. A lucky dozen"
    new "35. Şanslı bir düzine"


    old "Finish 12 endings"
    new "12 sonu bitir"


    old "ORNY"
    new "ORNY"


    old "H"
    new "H"


    old "UNGRY"
    new "UNGRY"


    old "APPY"
    new "APPY"

  # Decompiled by unrpyc_v1.2.0-alpha: https://github.com/CensoredUsername/unrpyc
