

translate russian strings:


    old "NAME"
    new "НАЗВАНИЕ"


    old "DATE"
    new "ДАТА"


    old "Unnamed End Game Save"
    new "Безымянное конечное сохранение"


    old "No end game save from week [FLSS.week.prev] has been detected"
    new "Не удалось обнаружить сохранения игры с недели [FLSS.week.prev]"


    old "Please open Fetish Locator week [FLSS.week.prev] and SAVE at the end game save screen"
    new "Пожалуйста, откройте Фетиш Локатор Неделя [FLSS.week.prev] и СОХРАНИТЕСЬ в конце игры"


    old "Due to some new changes in Android OS, to import your save from Week 2 you need to follow these instructions."
    new "В связи с некоторыми изменениями в Android OS, чтобы импортировать сохранения недели 2, Вам необходимо следовать этим инструкциям."


    old "1. Close the game and find the Week save file named \"FetishLocator\" in - \"InternalStorage/RenPy/persistent/\""
    new "1. Закройте игру и найдите файл сохранения с именем \"FetishLocator\" в - \"InternalStorage/RenPy/persistent/\""


    old "2. Copy the \"FetishLocator\" file to the following folder (replace existing files if any) -"
    new "2. Скопируйте файл \"FetishLocator\" в следующую папку (с заменой файлов) - "


    old "\"Internal storage/Android/data/com.patreon.fetishlocator.week3/files/saves/\"."
    new "\"Internal storage/Android/data/com.patreon.fetishlocator.week3/files/saves/\"."


    old "Contact us on {a=https://discord.gg/efmQRNtFks}Discord{/a} if you need help."
    new "Свяжитесь с нами в {a=https://discord.gg/efmQRNtFks}Discord{/a}, если Вам нужна помощь."


    old "Or"
    new "Или"


    old "{u}Fill the questionnaire to play week [FLSS.week.cur] without loading a save{/u}"
    new "{u}Ответьте на вопросы, чтобы начать играть в неделю [FLSS.week.cur] без загрузки сохранения{/u}"


    old "Your device can't save the game progress through different weeks. Please, allow FL App to write on the external storage."
    new "Ваше устройство не может сохранять игровой прогресс разных недель. Пожалуйста, предоставьте приложению права записи на внешнее хранилище."


    old "On most devices you can set the permissions on the Setting app."
    new "На большинстве устройств Вы можете установить разрешения в приложении Настроек."


    old "1. Go to - Settings -> Apps -> Fetish Locator Week [FLSS.week.cur] -> Permissions -> Storage."
    new "1. Перейдите в Настройки -> Приложения -> Fetish Locator Week [FLSS.week.cur] -> Разрешения -> Хранилище."


    old "2. Set the permission to -> \"Allow management of all files\"."
    new "2. Установите разрешение на -> \"Разрешить управление всеми файлами\"."


    old "Now the game should work."
    new "Теперь игра должна работать."


    old "Your device cannot save the game progress through different weeks. Please, follow these instructions to fix this issue."
    new "Ваше устройство не может сохранять игровой прогресс разных недель. Пожалуйста, следуйте этим инструкциям, чтобы решить эту проблему."


    old "1. Enable \"Show hidden items\" option in Windows Explorer."
    new "1. Включите опцию \"Показывать скрытые элементы\" в Проводнике Windows"


    old "2. Go to - \"C:/Users/username/AppData/Roaming/RenPy/persistent/\" folder."
    new "2. Перейдите по пути - \"%appdata%/RenPy/persistent/\"."


    old "3. Create a backup of the file \"FetishLocator\" and delete it."
    new "3. Создайте резервную копию файла \"FetishLocator\" и удалите его."


    old "4. After launching the game once, you can put the backed up \"FetishLocator\" file back to it's location to get your Previous Week's saves back."
    new "4. После единократного запуска игры, Вы можете положить копию файла сохранения обратно по тому же пути, для доступа к Вашим сохранениям предыдущих Недель."


    old "Continue playing"
    new "Продолжить играть"


    old "If you decide to continue, you might not be able to load saves from Week-2. You won't be notified again. Are you sure?"
    new "Если Вы решите продолжить, Вы не сможете загрузить сохранения второй недели. Вы больше не увидете это уведомление. Вы уверены?"


    old "Close the game"
    new "Выйти из игры"


    old "Save your game now"
    new "Сохранить игру"


    old "{u}Done{/u}"
    new "{u}Готово{/u}"


    old "points: "
    new "очки: "


    old "Welcome to"
    new "Добро пожаловать в"


    old "Please enter your name here"
    new "Пожалуйста, введите своё имя"


    old "Please enter your Fetish Locator username here"
    new "Пожалуйста, введите свой будущий ник в Фетиш Локаторе"


    old "Please choose your preferences"
    new "Пожалуйста, выберите ваши предпочтения"


    old "Yes to all"
    new "\"Да\" на всё"


    old "Please select whom you like"
    new "Пожалуйста, выберите, кто Вам нравится"


    old "Go back and enable {b}Watersports{/b} fetish to be able to select Min"
    new "Вернитесь назад и включите {b}Водные виды спорта{/b} для выбора Мин"


    old "Go back and enable {b}Trans Content{/b} to be able to select Lyssa"
    new "Вернитесь назад и включите {b}Транс Контент{/b} для выбора Лиссы"


    old "Please answer the questions"
    new "Пожалуйста, ответьте на вопросы"


    old "AmRose Romance or Slave route?"
    new "Любовный роман, или Личная Секс Рабыня?"


    old "Romance"
    new "Роман"


    old "Slave"
    new "Рабыня"


    old "Did you kiss AmRose?"
    new "Вы целовались с АмРоуз?"


    old "Do you want to dominate Lyssa?"
    new "Вы хотите доминировать над Лиссой?"


    old "Did you say that you love Lydia?"
    new "Вы признавались Лидии в любви?"


    old "Select all"
    new "Выбрать всё"


    old "Did you agree to have kids with Stacy?"
    new "Вы согласились завести детей со Стейси?"


    old "Did you creampie Nora and agree to be the father?"
    new "Вы кончили в Нору и согласились стать отцом?"


    old "Did you ask Vanessa to wear shoes with cum?"
    new "Вы просили Ванессу надеть обувь со спермой?"


    old "Did you give the password to Hana?"
    new "Вы дали пароль Хане?"


    old "Did you let Pete hit on Samiya?"
    new "Вы позволили Питу заигрывать с Самией?"


    old "How do you want to name your save?"
    new "Как Вы хотите назвать Ваше сохранение?"


    old "How do you want to rename this page?"
    new "Как Вы хотите переименовать эту страницу?"


    old "Undo"
    new "Отмена"


    old "Fetish Locator is intended for a mature audience\nof 18 years or older. Please abide by your local laws.\nAll characters are 18 years or older. No exceptions.\nThe characters in Fetish Locator explore relationships and their sexuality in both healthy and unhealthy ways. Some of these may be offensive to some players. Viewer discretion is advised. To that end, much of the content is optional and often there are alternative paths available. This is necessary to the story. The content explored should not be treated as an endorsement of particular fetishes, kinks, or patterns of behaviour. Please explore your own relationships and sexuality responsibly.\n\nEnjoy!"
    new "Фетиш Локатор предназначен для взрослой аудитории\nот 18 лет и старше. Пожалуйста, соблюдайте местные законы.\nВсе персонажи - от 18 лет и старше. Никаких исключений.\nПерсонажи в Фетиш Локаторе исследуют отношения и свою сексуальность как здоровыми, так и нездоровыми способами. Некоторые из них могут быть оскорбительными для некоторых игроков. Зрителю рекомендуется действовать по своему усмотрению. В связи с этим большая часть контента является необязательной, и часто существуют альтернативные пути. Это необходимо для сюжета. Исследуемое содержание не должно рассматриваться как одобрение определенных фетишей, извращений или моделей поведения. Пожалуйста, исследуйте свои собственные отношения и сексуальность ответственно.\n\nНаслаждайтесь!"


    old "Skip Recap\nof Week-1"
    new "Пропустить повтор\nWeek-1"


    old "Skip Recap\nof Week-2"
    new "Пропустить повтор\nWeek-2"
# Decompiled by unrpyc: https://github.com/CensoredUsername/unrpyc
# TODO: Translation updated at 2024-11-01 02:53

translate russian strings:

    # game/code/screens/screens_fl.rpy:285
    old "Touch input supported"
    new "Сенсорный ввод поддерживается"

    # game/code/screens/screens_fl.rpy:286
    old "Navigation"
    new "Навигация"

    # game/code/screens/screens_fl.rpy:287
    old "Pause Menu"
    new "Меню Паузы"

    # game/code/screens/screens_fl.rpy:288
    old "Quick Save"
    new "Б.Сохр."

    # game/code/screens/screens_fl.rpy:290
    old "Roll Back"
    new "Отк. Назад"

    # game/code/screens/screens_fl.rpy:293
    old "{b}A{/b} - Select"
    new "{b}A{/b} - Выбрать"

    # game/code/screens/screens_fl.rpy:294
    old "{b}B{/b} - Go Back"
    new "{b}B{/b} - Назад"

    # game/code/screens/screens_fl.rpy:295
    old "{b}X{/b} - Skip"
    new "{b}X{/b} - Пропуск"

    # game/code/screens/screens_fl.rpy:296
    old "{b}Y{/b} - Hide UI"
    new "{b}Y{/b} - Скрыть UI"

    # game/code/screens/screens_fl.rpy:297
    old "Roll Forward"
    new "Отк. Вперёд"

    # game/code/screens/screens_fl.rpy:300
    old "You can find this layout in the {b}Help{/b} menu"
    new "Этот макет можно найти в меню {b}Помощь{/b}."

    # game/code/screens/screens_fl.rpy:302
    old "CLOSE"
    new "ЗАКРЫТЬ"

    # game/code/screens/screens_fl.rpy:1343
    old "Sync your saves using Ren'Py Sync server"
    new "Синхронизируйте Ваши сохранения с помощью Ren'Py Sync"

    # game/code/screens/screens_fl.rpy:1345
    old "Upload Saves"
    new "Загрузить Сохранения"

    # game/code/screens/screens_fl.rpy:1346
    old "Download Saves"
    new "Скачать Сохранения"

    # game/code/screens/screens_fl.rpy:1394
    old "{u}Get Taboo University Book 1 on Steam{/u}"
    new "{u}Получите Taboo University Book 1 из Steam{/u}"

# TODO: Translation updated at 2025-02-20 22:03

translate russian strings:

    # game/code/screens/screens_fl.rpy:1396
    old "{u}Get Taboo University Book 1 on GOG{/u}"
    new "{u}Получите Taboo University Book 1 из GOG{/u}"

