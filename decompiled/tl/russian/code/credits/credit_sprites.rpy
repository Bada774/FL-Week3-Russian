# TODO: Translation updated at 2023-07-01 17:14

translate russian strings:

    # game/code/credits/credit_sprites.rpy:27
    old "Story Writing -"
    new "Сценарий -"

    # game/code/credits/credit_sprites.rpy:42
    old "Art -"
    new "Арты -"

    # game/code/credits/credit_sprites.rpy:62
    old "Programming -"
    new "Программирование -"

    # game/code/credits/credit_sprites.rpy:75
    old "Sound design -"
    new "Звуковое оформление -"

    # game/code/credits/credit_sprites.rpy:85
    old "Music -"
    new "Музыка -"

    # game/code/credits/credit_sprites.rpy:98
    old "Testing, Proofreading\n& Translation -"
    new "Тестирование, Пруфридинг\n& Перевод -"

    # game/code/credits/credit_sprites.rpy:110
    old "Russian Translation -"
    new "Русская локализация -"

    # game/code/credits/credit_sprites.rpy:120
    old "Special Thanks -"
    new "Специальная благодарность -"

    # game/code/credits/credit_sprites.rpy:132
    old "This game was only\nmade possible with\nall the support we have\nfrom Patreon\nand SubscribeStar"
    new "Эта игра стала \nвозможной только \nблагодаря \nподдержке со \nстороны Patreon \nи SubscribeStar \nподписчиков"

    # game/code/credits/credit_sprites.rpy:140
    old "Huge thanks to\nall of our fans\nand everyone who\nsupported us and\nhelped to bring\nthis story to life"
    new "Огромное спасибо \nвсем нашим поклонникам \nи всем, кто поддерживал нас \nи помог воплотить эту \nисторию в жизнь"

    # game/code/credits/credit_sprites.rpy:149
    old "SKIP"
    new "ПРОПУСТИТЬ"

    # game/code/credits/credit_sprites.rpy:177
    old "Thank you for playing the game!"
    new "Спасибо, что играли в эту игру!"

    # game/code/credits/credit_sprites.rpy:178
    old "We'll see you soon!"
    new "Скоро увидимся!"

# TODO: Translation updated at 2023-10-22 14:48

translate russian strings:

    # game/code/credits/credit_sprites.rpy:169
    old "Russian localization - Bada774"
    new "Русская локализация - Bada774"

