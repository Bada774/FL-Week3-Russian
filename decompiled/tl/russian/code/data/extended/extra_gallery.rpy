

translate russian strings:


    old "01. Worst Nightmare"
    new "01. Худший кошмар"


    old "Unlock the hidden 19th ending"
    new "Разблокируйте спрятанную 19-ую концовку"


    old "02. GG EZ Videogame"
    new "02. GG EZ"


    old "Get a high score in the minigame"
    new "Побейте рекорд в мини-игре"


    old "03. Somebody that I Used to Know"
    new "03. Кто-то, кого я знал"


    old "Unlock Lydia's music video"
    new "Разблокируйте музыкальное видео Лидии"


    old "04. Full Course"
    new "04. Полный курс"


    old "Unlock 18 endings"
    new "Разблокируйте 18 концовок"


    old "05. Front Nine"
    new "05. Первая Девятка"


    old "Unlock 9 endings"
    new "Разблокируйте 9 концовок"


    old "06. What a Wild Ride"
    new "06. Какая Дикая Поездка"


    old "Complete all 6 base endings"
    new "Разблокируйте все 6 базовые концовки"


    old "07. WTF Bro?"
    new "07. ЧЗХ Бро?"


    old "See everything you never wanted to"
    new "Увидьте всё, чего Вы никогда не хотели бы видеть"


    old "08. Hacker"
    new "08. Хакер"


    old "View all the secret door codes"
    new "Найдите все секретные коды дверей"


    old "09. In the Beginning..."
    new "09. В начале..."


    old "Make it all the way to Day 15"
    new "Пройдите весь путь до Дня 15"


    old "10. Release the Kraken"
    new "10. Выпустить Кракена"


    old "Get free from the cage"
    new "Освободитесь из клетки"


    old "11. Sudo Admin"
    new "11. Sudo Admin"


    old "Enter the server room"
    new "Войдите в сервеную"


    old "12. rm -rf"
    new "12. rm -rf"


    old "Destroy the app... or not"
    new "Уничтожьте приложение... или нет"


    old "13. The People Versus..."
    new "13. Люди Против...."


    old "Go to the court trial"
    new "Попадите на судебный процесс"


    old "14. Cherry Popped"
    new "14. Cherry Popped"


    old "Finish your first time"
    new "Закончите свой первый раз"


    old "15. Miss White, with the Candlestick"
    new "15. Мисс Уайт, с подсвечником"


    old "Discover who is behind the app on the LC path"
    new "Узнайте, кто стоит за приложением в сюжетном пути LC"


    old "16. Keyser Soze"
    new "16. Кейзер Сёзе"


    old "Discover who is behind the app on the NTR path"
    new "Узнайте, кто стоит за приложением в сюжетном пути NTR"


    old "17. Real Genius"
    new "17. Настоящий Гений"


    old "Finish final exams perfectly"
    new "Сдайте выпускные экзамены на отлично"


    old "18. Keep Pounding That Gavel"
    new "18. Продолжай бить молотком"


    old "Get both scenes with the judge"
    new "Найдите обе сцены с Судьей"


    old "19. What Are Friends For"
    new "19. Для чего нужны друзья"


    old "Knock up your best friend's gal"
    new "Уведите девушку своего лучшего друга"


    old "20. Are You Not Entertained"
    new "20. Тебе не интересно"


    old "Play all three scenes at the amusement park"
    new "Просмотрите все три сцены в парке развлечений"


    old "21. Oh, the Humanity!"
    new "21. Ох, человечество!"


    old "Play all three versions of Lyssa's Wednesday scenes"
    new "Просмотрите все три версии сцен Лиссы в Среду"


    old "22. Why Would You Do That"
    new "22. Зачем тебе это делать"


    old "Don't remove the cage, even though you could"
    new "Не снимайте клетку, даже если можете"


    old "23. Fantastic Fivesome"
    new "23. Фантастическая пятерка"


    old "Maria and some friends are very thankful"
    new "Мария и несколько друзей очень благодарны"


    old "24. Craving Pickles and Ice-cream"
    new "24. Маринованные огурцы и Мороженное"


    old "Impregnate every possible person in the queen's harem"
    new "Оплодотворите всех, кого можно в гареме королевы"


    old "25. I Need Sexual Healing"
    new "25. Мне необходимо Секс-исцеление"


    old "Helped all 3 couples at the ski resort"
    new "Помогите всем трём парам на курорте"


    old "26. This Magic Moment"
    new "26. Магический момент"


    old "Went on the ferris wheel with Daisy"
    new "Пойдите на колесо обозрения с Дейзи"


    old "27. May I panic now, Mistress?"
    new "27. Можно паниковать, Госпожа?"


    old "Braved the funhouse with Dahlia"
    new "Переживите веселье с Далией"


    old "28. Fifty-Five Foot Club"
    new "28. Клуб пятидесяти-пяти футов"


    old "Join Polly and Nora on the ferris wheel"
    new "Присоединитесь к Полли и Норе на колесе обозрения"


    old "29. That Was Cathartic... for Them"
    new "29. Это был катарсис... для Них"


    old "Fix Dahlia and Samiya's relationship"
    new "Примерите Далию и Самию"


    old "HIGH SCORE :\n[persistent.minigame_max_score]"
    new "РЕКОРД :\n[persistent.minigame_max_score]"


    old "HOW MANY ENDINGS HAVE YOU UNLOCKED?"
    new "СКОЛЬКО КОНЦОВОК ВЫ РАЗБЛОКИРОВАЛИ?"


    old "9 ENDINGS"
    new "9 КОНЦОВОК"


    old "YOU GOTTA PUMP THOSE NUMBERS UP.\nTHOSE ARE ROOKIE NUMBERS!"
    new "ВЫ ДОЛЖНЫ УЛУЧШИТЬ РЕЗУЛЬТАТ.\nЭТОТ ДЛЯ НУБОВ!"


    old "6 endings\naren't enough.\nI need all 18 now!!!"
    new "6 концовок\nне достаточно.\nМне нужны все 18!!!"


    old "{b}CHANGE MY MIND{/b}"
    new "{b}ПЕРЕУБЕДИ МЕНЯ{/b}"


    old "I WANT TO KEEP\nFETISH LOCATOR"
    new "Я ХОЧУ СОХРАНИТЬ\nФЕТИШ ЛОКАТОР"


    old "TO MAKR SURE NO ONE\nWILL BE BLACKMAILED"
    new "ЧТОБЫ БЫТЬ УВЕРЕННЫМ\nЧТО НИКОГО НЕ БУДУТ\nШАНТАЖИРОВАТЬ"


    old "YOU'RE GOING TO USE\nIT FOR GOOD, RIGHT?"
    new "ТЫ СОБИРАЕШЬСЯ ИСПОЛЬЗОВАТЬ\nЕГО ВО ИМЯ ДОБРА, ВЕРНО?"


    old "RIGHT?"
    new "ВЕРНО?"


    old "THE FIRST ENDING\nFROM 18 TOTAL ENDINGS"
    new "ПЕРВАЯ КОНЦОВКА\nИЗ 18 КОНЦОВОК"


    old "IS THAT A REAL ENDING?"
    new "ЭТО НАСТОЯЩИЙ КОНЕЦ?"


    old "TRADE OFFER"
    new "ДЕЛОВОЕ ПРЕДЛОЖЕНИЕ"


    old "I receive:"
    new "Я получаю:"


    old "Cum in every pussy\nat the wedding"
    new "Кончить в каждую киску\nна свадьбе"


    old "You receive:"
    new "Вы получаете:"


    old "The whole\nharem pregnant"
    new "Весь гарем\nоплодотворён"



translate russian strings:


    old "30. What? You wanted more?"
    new "30. Что? Хочешь ещё?"


    old "Finish all 6 Endings from DLC-1"
    new "Завершите все 6 концовок DLC-1"


    old "31. Go Debbie Go"
    new "31. Давай Дебби, Давай"


    old "Have sex with Debbie"
    new "Займитесь сексом с Дебби"


    old "32. Fully Cooked"
    new "32. Полностью приготовленный"


    old "Get max points in Lydia's challenges in e07"
    new "Получите максимальное количество очков в испытаниях Лидии в Концовке 07."


    old "33. Full House"
    new "33. Full House"


    old "Play with all the characters in e04"
    new "Сыграйте со всеми персонажами в Концовке 04."


    old "34. Day and Night"
    new "34. День и Ночь"


    old "Finish e04 and e07"
    new "Завершите Концовки 04 и 07"


    old "35. A lucky dozen"
    new "35. Счастливая дюжина"


    old "Finish 12 endings"
    new "Завершите 12 концовок"


    old "TO MAKE SURE NO ONE\nWILL BE BLACKMAILED"
    new "ЧТОБЫ УБЕДИТЬСЯ, ЧТО НИКОГО\nНЕ БУДУТ ШАНТАЖИРОВАТЬ"


    old "ORNY"
    new "ORNY"


    old "H"
    new "H"


    old "UNGRY"
    new "UNGRY"


    old "APPY"
    new "APPY"
# Decompiled by unrpyc: https://github.com/CensoredUsername/unrpyc
# TODO: Translation updated at 2024-11-01 02:53

translate russian strings:

    # game/code/data/extended/extra_gallery.rpy:51
    old "Finish Ending 4 and Ending 7"
    new "Завершите Концовку 4 и Концовку 7"

    # game/code/data/extended/extra_gallery.rpy:60
    old "36. Don't threaten me with a good time"
    new "36. Не угрожай мне весельем"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Finish all 6 Endings from DLC-2"
    new "Завершите все 6 концовок DLC-2"

    # game/code/data/extended/extra_gallery.rpy:60
    old "37. I saw, I came, I conquered"
    new "37. Я увидел, Я пришёл, Я завоевал"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Have all Characters in last orgy in Ending 9"
    new "Получите всех Персонажей в последней оргии в Концовке 9."

    # game/code/data/extended/extra_gallery.rpy:60
    old "38. Golden Pass"
    new "38. Золотой Перевал"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Have all Characters in Ending 15"
    new "Получите всех Персонажей в Концовке 15"

    # game/code/data/extended/extra_gallery.rpy:60
    old "39. She belongs in a museum"
    new "39. Её место в музее"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Find golden Lyssa (Ending 3)"
    new "Найдите золотую Лиссу (Концовка 3)"

    # game/code/data/extended/extra_gallery.rpy:60
    old "40. Do the Pulp Fiction Thing"
    new "40. Повторите Штуку из \"Криминального чтива\""

    # game/code/data/extended/extra_gallery.rpy:60
    old "Do the pulp fiction thing (Ending 18)"
    new "Повторите Штуку из \"Криминального чтива\" (Концовка 18)"

    # game/code/data/extended/extra_gallery.rpy:60
    old "41. Backdoor Cat Girl"
    new "41. Задний ход Кошкодевочки"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Backdoor Cat Girl (Ending 16)"
    new "Задний ход Кошкодевочки (Концовка 16)"

    # game/code/data/extended/extra_gallery.rpy:60
    old "42. Pee Bootcamp"
    new "42. Мочевой Учебный Лагерь"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Begin Olivia's Training"
    new "Начните обучение Оливии"

    # game/code/data/extended/extra_gallery.rpy:60
    old "43. Choo-choo"
    new "43. Чух-чух"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Choo-choo (Ending 5)"
    new "Чух-чух (Концовка 5)"

    # game/code/data/extended/extra_gallery.rpy:60
    old "44. Londyn Twice"
    new "44. Дважды Лондин"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Meet Londyn in Ending 9 and Ending 18"
    new "Встретьтесь с Лондин в Концовке 9 и Концовке 18"

    # game/code/data/extended/extra_gallery.rpy:60
    old "45. I've seen all the possible outcomes"
    new "45. Я видел все возможные исходы"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Finish all 18 Endings"
    new "Завершите все 18 Концовок"

    # game/code/data/extended/extra_gallery.rpy:802
    old "SPIT ON THAT THANG!"
    new "ПЛЮНЬ НА ЭТУ ВЕЩЬ!"

    # game/code/data/extended/extra_gallery.rpy:823
    old "WHEN HE MANAGES TO GET EVERYONE TO COME TO YOUR FANCY NEW SPA"
    new "КОГДА ОН СМОГ ЗАТАЩИТЬ ВСЕХ В ВАШ НОВЫЙ МОДНЫЙ СПА"

    # game/code/data/extended/extra_gallery.rpy:845
    old "Fucking\na girl"
    new "Трахать\nдевочку"

    # game/code/data/extended/extra_gallery.rpy:847
    old "Fucking\na catgirl"
    new "Трахать\nкошкодевочку"

    # game/code/data/extended/extra_gallery.rpy:861
    old "Hard to\nswallow\npills"
    new "Трудно\глотаемые\nтаблетки"

    # game/code/data/extended/extra_gallery.rpy:863
    old "He will never\nrun out of pee"
    new "У него\nбесконечная моча"

    # game/code/data/extended/extra_gallery.rpy:901
    old "NEXT STOP"
    new "СЛЕДУЮЩАЯ ОСТАНОВКА"

    # game/code/data/extended/extra_gallery.rpy:903
    old "PENETRATION STATION"
    new "СТАНЦИЯ ПРОНИКНОВЕНИЕ"

    # game/code/data/extended/extra_gallery.rpy:917
    old "They don't know that\nI appear in two endings"
    new "Они не знают\nчто я появляюсь в двух концовках"

