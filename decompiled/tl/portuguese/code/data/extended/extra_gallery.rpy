translate portuguese strings:


    old "01. Worst Nightmare"
    new "01. Pior Pesadelo"


    old "Unlock the hidden 19th ending"
    new "Destravar o final secreto 19"


    old "02. GG EZ Videogame"
    new "02. GG Jogo EZ"


    old "Get a high score in the minigame"
    new "Consiga muitos pontos no minigame"


    old "03. Somebody that I Used to Know"
    new "03. Alguém que eu Conhecia"


    old "Unlock Lydia's music video"
    new "Libere o vídeo musical da Lydia"


    old "04. Full Course"
    new "04. Tudo Completado"


    old "Unlock 18 endings"
    new "Libere 18 Finais"


    old "05. Front Nine"
    new "05. Primeiros Nove"


    old "Unlock 9 endings"
    new "Libere 9 finais"


    old "06. What a Wild Ride"
    new "06. Que Viagem Louca"


    old "Complete all 6 base endings"
    new "Complete todos os 6 finais básicos"


    old "07. WTF Bro?"
    new "07. WTF Cara?"


    old "See everything you never wanted to"
    new "Veja tudo que nunca quis ver"


    old "08. Hacker"
    new "08. Hacker"


    old "View all the secret door codes"
    new "Veja todos os códigos secretos da porta"


    old "09. In the Beginning..."
    new "09. No Começo..."


    old "Make it all the way to Day 15"
    new "Chegue até o Dia 15"


    old "10. Release the Kraken"
    new "10. Sotem o Kraken"


    old "Get free from the cage"
    new "Liberte-se da jaula"


    old "11. Sudo Admin"
    new "11. Sudo Admin"


    old "Enter the server room"
    new "Entre na sala do servidor"


    old "12. rm -rf"
    new "12. rm -rf"


    old "Destroy the app... or not"
    new "Destrua o aplicativo... ou não"


    old "13. The People Versus..."
    new "13. Governo Contra..."


    old "Go to the court trial"
    new "Vá para o processo no tribunal"


    old "14. Cherry Popped"
    new "14. Desvirginado"


    old "Finish your first time"
    new "Complete sua primeira vez."


    old "15. Miss White, with the Candlestick"
    new "16. Senhorita White, com o Castiçal"


    old "Discover who is behind the app on the LC path"
    new "Descubra quem está por trás do aplicativo no caminho LC"


    old "16. Keyser Soze"
    new "16 Keyser Soze"


    old "Discover who is behind the app on the NTR path"
    new "Descubra quem está por trás do aplicativo no caminho NTR"


    old "17. Real Genius"
    new "17. Gênio Verdadeiro"


    old "Finish final exams perfectly"
    new "Termine as provas finais acertando tudo"


    old "18. Keep Pounding That Gavel"
    new "18. Continua Metendo Esse Martelinho"


    old "Get both scenes with the judge"
    new "Consiga ambas as cenas com a juíza"


    old "19. What Are Friends For"
    new "19. Para Que Servem Os Amigos"


    old "Knock up your best friend's gal"
    new "Engravide a garota do seu melhor amigo"


    old "20. Are You Not Entertained"
    new "20. Será Que Você Não Está Entretido"


    old "Play all three scenes at the amusement park"
    new "Jogue todas as três cenas no parque de diversão"


    old "21. Oh, the Humanity!"
    new "21. Ah, a Humanidade!"


    old "Play all three versions of Lyssa's Wednesday scenes"
    new "Jogue todas as três versões das cenas de Quarta-feira da Lyssa"


    old "22. Why Would You Do That"
    new "22. Por Que Você Faria Isso"


    old "Don't remove the cage, even though you could"
    new "Não remova a jaula, mesmo podendo"


    old "23. Fantastic Fivesome"
    new "23. Quinteto Fantástico"


    old "Maria and some friends are very thankful"
    new "Maria e alguns amigos agradecem muito"


    old "24. Craving Pickles and Ice-cream"
    new "24. Querendo Picles e Sorvete"


    old "Impregnate every possible person in the queen's harem"
    new "Engravide todas as pessoas possíveis no harém da rainha"


    old "25. I Need Sexual Healing"
    new "25. Preciso De Uma Cura Sexual"


    old "Helped all 3 couples at the ski resort"
    new "Ajudou todos os 3 casais no resort de esqui"


    old "26. This Magic Moment"
    new "26. Esse Momento Mágico"


    old "Went on the ferris wheel with Daisy"
    new "Foi para a roda gigante com Daisy"


    old "27. May I panic now, Mistress?"
    new "27. Posso Entrar Em Pânico Agora, Mestra?"


    old "Braved the funhouse with Dahlia"
    new "Desbravejou na casa de diversões com Dahlia"


    old "28. Fifty-Five Foot Club"
    new "28. Clube Dos 30 Metros De Algura"


    old "Join Polly and Nora on the ferris wheel"
    new "Vá com Polly e Nora para a roda gigante"


    old "29. That Was Cathartic... for Them"
    new "29. Isso foi Catártico... para Elas"


    old "Fix Dahlia and Samiya's relationship"
    new "Conserte o relacionamento de Dahlia e Samiya"


    old "HOW MANY ENDINGS HAVE YOU UNLOCKED?"
    new "QUANTOS FINAIS VOCÊ LIBEROU?"


    old "9 ENDINGS"
    new "9 FINAIS"


    old "YOU GOTTA PUMP THOSE NUMBERS UP.\nTHOSE ARE ROOKIE NUMBERS!"
    new "VOCÊ PRECISA AUMENTAR ESSES NÚMEROS.\nESSES NÚMEROS ESTÃO BAIXOS!"


    old "6 endings\naren't enough.\nI need all 18 now!!!"
    new "6 finais não\nsão o suficiente.\nPreciso de todos\nos 18 agora!!!"


    old "{b}CHANGE MY MIND{/b}"
    new "FAÇA EU MUDAR DE IDEIA"


    old "I WANT TO KEEP\nFETISH LOCATOR"
    new "EU QUERO FICAR COM\nO FETISH LOCATOR"


    old "TO MAKE SURE NO ONE\nWILL BE BLACKMAILED"
    new "PARA ME CERTIFICAR QUE\nNINGUÉM VAI SER CHANTAGEADO"


    old "YOU'RE GOING TO USE\nIT FOR GOOD, RIGHT?"
    new "VOCÊ VAI USAR\nELE PARA O BEM, NÉ?"


    old "RIGHT?"
    new "NÉ?"


    old "THE FIRST ENDING\nFROM 18 TOTAL ENDINGS"
    new "O PRIMEIRO FINAL\nDE 18 FINAIS TOTAIS"


    old "IS THAT A REAL ENDING?"
    new "ESSE É UM FINAL DE VERDADE?"


    old "TRADE OFFER"
    new "OFERTA DE TROCA"


    old "I receive:"
    new "Eu recebo:"


    old "Cum in every pussy\nat the wedding"
    new "Goze em todas as \nbocetas no casamento"


    old "You receive:"
    new "Você recebe:"


    old "The whole\nharem pregnant"
    new "O harém\ntodo grávido"


    old "HIGH SCORE :\n[persistent.minigame_max_score]"
    new "PONTUAÇÃO\nMÁXIMA :\n[persistent.minigame_max_score]"


    old "30. What? You wanted more?"
    new "30. O quê? Você queria mais?"


    old "Finish all 6 Endings from DLC-1"
    new "Termine todos os 6 Finais do DLC-1"


    old "31. Go Debbie Go"
    new "31. Vai Debbie Vai"


    old "Have sex with Debbie"
    new "Faça sexo com a Debbie"


    old "32. Fully Cooked"
    new "32. Bem Passado"


    old "Get max points in Lydia's challenges in e07"
    new "Consiga a pontuação máxima no desafio da Lydia no e07"


    old "33. Full House"
    new "33. Casa Cheia"


    old "Play with all the characters in e04"
    new "Jogue com todas personagens no e04"


    old "34. Day and Night"
    new "34. Dia e Noite"


    old "Finish e04 and e07"
    new "Complete e04 e e07"


    old "35. A lucky dozen"
    new "35. A dúzia da sorte"


    old "Finish 12 endings"
    new "Complete 12 finais"


    old "ORNY"
    new "OGOSA"


    old "H"
    new "F"


    old "UNGRY"
    new "AMINTA"


    old "APPY"
    new "ELIZ"


    old "Finish Ending 4 and Ending 7"
    new "Complete o Final 4 e Final 7"


    old "36. Don't threaten me with a good time"
    new "36. Não me ameace com coisa boa"


    old "Finish all 6 Endings from DLC-2"
    new "Termine todos os Finais do DLC-2"


    old "37. I saw, I came, I conquered"
    new "37. Eu vi, eu vim, eu conquistei"


    old "Have all Characters in last orgy in Ending 9"
    new "Tenha todos os Personagens na última orgia do Final 9"


    old "38. Golden Pass"
    new "38. Passe Dourado"


    old "Have all Characters in Ending 15"
    new "Tenha todos os Personagens no Final 15"


    old "39. She belongs in a museum"
    new "39. Ela pertence num museu"


    old "Find golden Lyssa (Ending 3)"
    new "Encontre a Lyssa dourada (Final 3)"


    old "40. Do the Pulp Fiction Thing"
    new "40. Faz a Parada do Pulp Fiction"


    old "Do the pulp fiction thing (Ending 18)"
    new "Faça a parada do pulp fiction (Final 18)"


    old "41. Backdoor Cat Girl"
    new "41. Backdoor Cat Girl"


    old "Backdoor Cat Girl (Ending 16)"
    new "Backdoor Cat Girl (Final 16)"


    old "42. Pee Bootcamp"
    new "42. Escola da Urina"


    old "Begin Olivia's Training"
    new "Comece o Treinamento da Olivia"


    old "43. Choo-choo"
    new "43. Choo-choo"


    old "Choo-choo (Ending 5)"
    new "Choo-choo (Final 5)"


    old "44. Londyn Twice"
    new "44. Londyn Dupla"


    old "Meet Londyn in Ending 9 and Ending 18"
    new "Encontre Londyn no Final 9 e Final 18"


    old "45. I've seen all the possible outcomes"
    new "45. Vi todos resultados possíveis"


    old "Finish all 18 Endings"
    new "Complete todos os 18 Finais"


    old "SPIT ON THAT THANG!"
    new "COSPE NESSA COISA!"


    old "WHEN HE MANAGES TO GET EVERYONE TO COME TO YOUR FANCY NEW SPA"
    new "QUANDO ELE FAZ TODO MUNDO VIR NO SEU NOVO SPA CHIQUE"


    old "Fucking\na girl"
    new "Fodendo\numa garota"


    old "Fucking\na catgirl"
    new "Fodendo\numa garota gato"


    old "Hard to\nswallow\npills"
    new "Difícil\nde\naceitar"


    old "He will never\nrun out of pee"
    new "Nunca vai acabar\na urina dele"


    old "NEXT STOP"
    new "PRÓXIMA PARADA"


    old "PENETRATION STATION"
    new "ESTAÇÃO DA PENETRAÇÃO"


    old "They don't know that\nI appear in two endings"
    new "Não fazem ideia que\neu apareço em dois finais"
# Decompiled by unrpyc: https://github.com/CensoredUsername/unrpyc
