translate french strings:


    old "PLAY"
    new "JOUER"


    old "LOAD"
    new "CHARGE"


    old "BONUS"
    new "BONUS"


    old "SETTINGS"
    new "PREFS"


    old "LANGUAGE"
    new "LANGUE"


    old "INFO"
    new "INFO"


    old "Support us on Patreon"
    new "Soutiens-nous sur Patreon"


    old "Join us on Discord"
    new "Rejoins-nous sur Discord"


    old "Quit Game"
    new "Quitte le jeu"


    old "Get a Lovense toy"
    new "Prends un jouet Lovense"


    old "Wishlist Taboo University on Steam"
    new "Liste de souhaits Taboo University sur Steam"


    old "Try our new game Taboo University"
    new "Essaie notre nouveau jeu Taboo University"


    old "Get free DLC containing 6 endings from Steam"
    new "Obtenez un DLC gratuit contenant 6 fins sur Steam"


    old "ANSWER THE QUESTIONNAIRE AND START A NEW GAME"
    new "RÉPONDS AU QUESTIONNAIRE ET COMMENCE UN NOUVEAU JEU"


    old "LOAD A SAVE FROM WEEK 2"
    new "CHARGE UNE SAUVEGARDE DE LA SEMAINE 2"


    old "ABOUT"
    new "A PROPOS"


    old "HELP"
    new "A L'AIDE"


    old "CG\nGALLERY"
    new "CG\nGALERIE"


    old "REPLAY ROOM"
    new "SALLE DE REPLAY"


    old "BONUS CONTENT"
    new "CONTENU BONUS"

translate french strings:


    old "Buy Taboo University on Steam"
    new "Acheter Taboo University sur Steam"

translate french strings:


    old "Get DLC-2 containing 6 additional endings from Steam"
    new "Obtenez le DLC-2 contenant 6 fins supplémentaires de Steam"

translate french strings:


    old "Buy Taboo University on GOG"
    new "Achetez Taboo University sur GOG"


    old "Watch Fetish Locator: S&M Studio trailer"
    new "Regardez Fetish Locator: bande-annonce de S&M Studio"
# Decompiled by unrpyc: https://github.com/CensoredUsername/unrpyc
