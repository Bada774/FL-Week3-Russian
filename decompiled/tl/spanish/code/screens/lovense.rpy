
translate spanish lovense_test_9e0ee773:


    "Prueba 1 (2)"


translate spanish lovense_test_6cca6db3:


    "Prueba 2 (20)"


translate spanish lovense_test_fa719a86:


    "Prueba de vibración 0"


translate spanish lovense_test_d7e7b816:


    "Prueba de la bomba BAJA"


translate spanish lovense_test_14a8f7e0:


    "Prueba de la bomba MAX"


translate spanish lovense_test_30b42ad1:


    "Prueba del modelo"


translate spanish lovense_test_37bd1745:


    "Vibración + Prueba de rotación"


translate spanish lovense_test_8ab253a3:


    "Detener prueba"

translate spanish strings:


    old "Please make sure both FL and Lovense Remote app are on the same LAN."
    new "Asegúrese de que tanto Fetish Locator como la aplicación remota de Lovense están conectados a la misma red LAN."


    old "Follow the steps to enable Game Mode in Lovense Remote app and connect your toy to FL."
    new "Sigue los pasos para activar el Modo Juego en la aplicación remota de Lovense y conecta tú juguete a Fetish Locator."


    old "1. Click on the \"Me\" tab"
    new "1. Haga clic en la pestaña \"Yo\""


    old "2. Then click on \"Settings\""
    new "2. Luego haga clic en \"Configuración\""


    old "3. Click the button to enable \"Game Mode\""
    new "3.  Haga clic en el botón para activar el \"Modo Juego\""


    old "4. Enter the \"Local IP\" and \"Http Port\" shown in the app"
    new "4. Introduzca la \"IP local\" y el \"Puerto http\" que aparecen en la aplicación."


    old "Local IP: "
    new "IP Local: "


    old "{}"
    new "{}"


    old "HTTP Port: "
    new "Puerto HTTP: "


    old "Connect"
    new "Conectar"


    old "Toy"
    new "Juguete"


    old "Your favorite toy now works with Fetish Locator!"
    new "¡Tú juguete favorito ahora funciona con Fetish Locator!"


    old "Get any Lovense toy with our exclusive 55% discount and sync it with Fetish Locator!"
    new "¡Consigue cualquier juguete Lovense con nuestro descuento exclusivo del 55 % y sincronízalo con Fetish Locator!"


    old "Limited time offer!"
    new "¡Oferta por tiempo limitado!"


    old "Get your discount now"
    new "Obtén tu descuento ahora"


    old "{u}Connect your toy!{/u}"
    new "{u}¡Conecta tú juguete!{/u}"


    old "Please enter your Local IP number:"
    new "Ingrese su número de IP local:"


    old "Please enter your HTTP Port number:"
    new "Ingrese su número del Puerto HTTP:"


    old "Back"
    new "Atrás"


    old "Save"
    new "Guardar"

translate spanish strings:


    old "Get a Lovense toy and enjoy the best way to experience Fetish Locator!"
    new "¡Consigue un juguete Lovense y disfruta de la mejor forma de experimentar Fetish Locator!"


    old "Get a toy now"
    new "Consigue un juguete ahora"

translate spanish strings:


    old "1. Click on the \"Discover\" tab"
    new "1. Haga clic en la pestaña \"Descubrir\""


    old "2. Then click on \"Game Mode\" option"
    new "2. A continuación, haga clic en la opción \"Modo de juego\""


    old "4. Enter the \"Local IP\" and \"Port\" shown in the app\nThen click Connect."
    new "4. Introduzca la \"IP local\" y el \"puerto\" que aparecen en la aplicación y haga clic en Conectar."
# Decompiled by unrpyc: https://github.com/CensoredUsername/unrpyc
