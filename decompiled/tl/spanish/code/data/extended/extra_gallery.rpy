

translate spanish strings:


    old "01. Worst Nightmare"
    new "01. La peor pesadilla"


    old "Unlock the hidden 19th ending"
    new "Desbloquea el final oculto nº 19"


    old "02. GG EZ Videogame"
    new "02. Videojuego GG EZ"


    old "Get a high score in the minigame"
    new "Consigue una puntuación alta en el minijuego"


    old "03. Somebody that I Used to Know"
    new "03. Alguien que yo conocía"


    old "Unlock Lydia's music video"
    new "Desbloquea el video musical de Lydia"


    old "04. Full Course"
    new "04. Curso completo"


    old "Unlock 18 endings"
    new "Desbloquea los 18 finales"


    old "05. Front Nine"
    new "05. Frente Nueve"


    old "Unlock 9 endings"
    new "Desbloquea el final 9"


    old "06. What a Wild Ride"
    new "06. Un viaje salvaje"


    old "Complete all 6 base endings"
    new "Completa los 6 finales básicos"


    old "07. WTF Bro?"
    new "07. ¿Qué mierda Hermano?"


    old "See everything you never wanted to"
    new "Ve todo lo que nunca quisiste"


    old "08. Hacker"
    new "08. Hacker"


    old "View all the secret door codes"
    new "Ver todos los códigos de puertas secretas"


    old "09. In the Beginning..."
    new "09. Al principio..."


    old "Make it all the way to Day 15"
    new "Llega hasta el día 15"


    old "10. Release the Kraken"
    new "10. Libera al Kraken"


    old "Get free from the cage"
    new "Libérate de la jaula"


    old "11. Sudo Admin"
    new "11. Administrador de Sudo"


    old "Enter the server room"
    new "Entrar en la sala de servidores"


    old "12. rm -rf"
    new "12. rm -rf"


    old "Destroy the app... or not"
    new "Destruir la aplicación... o no"


    old "13. The People Versus..."
    new "13. El pueblo contra..."


    old "Go to the court trial"
    new "Ir al juicio"


    old "14. Cherry Popped"
    new "14. cereza reventada"


    old "Finish your first time"
    new "Termina tu primera vez"


    old "15. Miss White, with the Candlestick"
    new "15. Señorita White, con el candelabro"


    old "Discover who is behind the app on the LC path"
    new "Descubre quién está detrás de la aplicación en la ruta LC"


    old "16. Keyser Soze"
    new "16. Keyser Soze"


    old "Discover who is behind the app on the NTR path"
    new "Descubre quién está detrás de la aplicación en la ruta NTR"


    old "17. Real Genius"
    new "17. Verdadero genio"


    old "Finish final exams perfectly"
    new "Terminar perfectamente los exámenes finales"


    old "18. Keep Pounding That Gavel"
    new "18. Sigue golpeando ese mazo"


    old "Get both scenes with the judge"
    new "Consigue ambas escenas con la jueza"


    old "19. What Are Friends For"
    new "19. Para que son los amigos"


    old "Knock up your best friend's gal"
    new "Deja embarazada a la chica de tu mejor amigo"


    old "20. Are You Not Entertained"
    new "20. ¿No estás entretenido?"


    old "Play all three scenes at the amusement park"
    new "Juega las tres escenas en el parque de diversiones"


    old "21. Oh, the Humanity!"
    new "21. ¡Oh, la humanidad!"


    old "Play all three versions of Lyssa's Wednesday scenes"
    new "Juega las tres versiones de las escenas de los miércoles de Lyssa"


    old "22. Why Would You Do That"
    new "22. Por qué harías eso"


    old "Don't remove the cage, even though you could"
    new "No te quites la jaula, aunque podrías"


    old "23. Fantastic Fivesome"
    new "23. Quinteto fantástico"


    old "Maria and some friends are very thankful"
    new "María y algunos amigos están muy agradecidos."


    old "24. Craving Pickles and Ice-cream"
    new "24. Antojo de pepinillos y helado"


    old "Impregnate every possible person in the queen's harem"
    new "Embarazar a todas las personas posibles en el harén de la reina"


    old "25. I Need Sexual Healing"
    new "25. Necesito sanación sexual"


    old "Helped all 3 couples at the ski resort"
    new "Ayudé a las 3 parejas en la estación de esquí"


    old "26. This Magic Moment"
    new "26. Este es un momento mágico"


    old "Went on the ferris wheel with Daisy"
    new "Subimos a la rueda de la fortuna con Daisy"


    old "27. May I panic now, Mistress?"
    new "27. ¿Puedo entrar en pánico ahora, Ama?"


    old "Braved the funhouse with Dahlia"
    new "Enfréntate a la casa de la risa con Dahlia"


    old "28. Fifty-Five Foot Club"
    new "28. Club de los cincuenta y cinco pies"


    old "Join Polly and Nora on the ferris wheel"
    new "Únete a Polly y Nora en la rueda de la fortuna"


    old "29. That Was Cathartic... for Them"
    new "29. Eso fue catártico ... para ellas"


    old "Fix Dahlia and Samiya's relationship"
    new "Arregla la relación de Dahlia y Samiya"


    old "HIGH SCORE :\n[persistent.minigame_max_score]"
    new "PUNTUACIÓN\nALTA :\n[persistent.minigame_max_score]"


    old "HOW MANY ENDINGS HAVE YOU UNLOCKED?"
    new "¿CUÁNTOS FINALES HAS DESBLOQUEADO?"


    old "9 ENDINGS"
    new "9 FINALES"


    old "YOU GOTTA PUMP THOSE NUMBERS UP.\nTHOSE ARE ROOKIE NUMBERS!"
    new "TIENES QUE AUMENTAR ESOS NÚMEROS.\n¡ESOS SON NÚMEROS DE NOVATO!"


    old "6 endings\naren't enough.\nI need all 18 now!!!"
    new "6 finales\no son suficientes.\n¡Necesito los 18 ahora!"


    old "{b}CHANGE MY MIND{/b}"
    new "{b}CAMBIO DE OPINIÓN{/b}"


    old "I WANT TO KEEP\nFETISH LOCATOR"
    new "QUIERO MANTENER EL\nLOCALIZADOR DE FETICHES"


    old "TO MAKE SURE NO ONE\nWILL BE BLACKMAILED"
    new "PARA ASEGURARSE DE QUE\nNADIE SERÁ CHANTAJEADO"


    old "YOU'RE GOING TO\nUSE IT FOR GOOD, RIGHT?"
    new "LO VA A UTILIZAR\nPARA BIEN, ¿VERDAD?"


    old "RIGHT?"
    new "¿BIEN?"


    old "THE FIRST ENDING\nFROM 18 TOTAL ENDINGS"
    new "EL PRIMER FINAL DE\nUN TOTAL DE 18 FINALES"


    old "IS THAT A REAL ENDING?"
    new "¿ES ESE UN FINAL VERDADERO?"


    old "I receive:"
    new "Recibo:"


    old "Cum in every pussy\nat the wedding"
    new "Corrida en cada\ncoño en la boda"


    old "You receive:"
    new "Tú recibes:"


    old "The whole\nharem pregnant"
    new "Todo el\nharem embarazado"


    old "YOU'RE GOING TO USE\nIT FOR GOOD, RIGHT?"
    new "VAS A USARLO PARA\nEL BIEN, ¿VERDAD?"


    old "TRADE OFFER"
    new "OFERTA DE INTERCAMBIO"


    old "30. What? You wanted more?"
    new "30. ¿Qué? ¿Querías más?"


    old "Finish all 6 Endings from DLC-1"
    new "Termina los 6 finales del DLC-1"


    old "31. Go Debbie Go"
    new "31. Vamos Debbie Vamos"


    old "Have sex with Debbie"
    new "Ten sexo con Debbie"


    old "32. Fully Cooked"
    new "32. Totalmente Cocinado"


    old "Get max points in Lydia's challenges in e07"
    new "Consigue el máximo de puntos en los desafíos de Lydia en el final 07"


    old "33. Full House"
    new "33. Casa llena"


    old "Play with all the characters in e04"
    new "Juega con todos los personajes del final 04"


    old "34. Day and Night"
    new "34. Día y Noche"


    old "Finish e04 and e07"
    new "Termina el final 04 y el final 07"


    old "35. A lucky dozen"
    new "35. Una docena de afortunados"


    old "Finish 12 endings"
    new "Termina los 12 finales"


    old "ORNY"
    new "ORNY"


    old "H"
    new "H"


    old "UNGRY"
    new "UNGRY"


    old "APPY"
    new "APPY"

  # Decompiled by unrpyc_v1.2.0-alpha: https://github.com/CensoredUsername/unrpyc
