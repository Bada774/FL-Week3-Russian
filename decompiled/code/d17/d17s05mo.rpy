label d17s05mo:

    scene black
    show screen scene_transistion(_("30 minutes later"))
    with Fade(0.9, 0.5, 0.5)
    pause
    hide screen scene_transistion
    play sound3 park fadein 3.0
    play sound sfx_motobike_stop fadein 1.0
    scene d17s05b-00-sy-mh-oliver-lyssa
    with Fade(0.5, 0.5, 0.9)
    mct "Didn't know Oliver was a biker."
    scene d17s05b-01-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_surprised_wow3 noloop
    mc "Didn't know you were into bikes."
    play voice3 oliver_hey_hiya noloop
    op "Oh, hi, [mcname]."
    scene d17s05b-02-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_um1 noloop
    op "I'm a—a, hobbyist, I suppose."
    scene d17s05b-03-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_arrogant_huh3 noloop
    mc "Huh, cool. She's pretty."
    scene d17s05b-04-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_heh2 noloop
    op "Heh, she is."
    play sound knockknock
    scene d17s05b-05-sy-mh-oliver-lyssa with dissolve
    pause
    play sound sfx_door_open1
    scene d17s05b-06-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_oh2 noloop
    mh "[mcname]!"
    $ renpy.music.set_volume(0.4, 0.0, "music")
    stop sound3 fadeout 5.0
    play voice2 d1s1_mmm noloop
    play voice3 lissa_moan1 noloop
    play sound maria_kiss1
    scene d17s05b-07-sy-mh-oliver-lyssa with dissolve
    play music tender_moment fadein 0.4
    pause
    scene d17s05b-08-sy-mh-oliver-lyssa with dissolve
    play voice3 dahlia_happy_hmm1 noloop
    mh "I was wondering what was taking you so long."
    scene d17s05b-09-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_yes_yeah5 noloop
    mc "Public transportation is a wonderful thing."
    scene d17s05b-10-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_shyoh noloop
    mh "Oh, you should've told me, I could've—"
    scene d17s05b-11-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_hey_hey3 noloop
    mc "I'm here now, aren't I?"
    play voice3 lissa_mmm1 noloop
    scene d17s05b-12-sy-mh-oliver-lyssa with dissolve
    pause
    scene d17s05b-13-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_oh noloop
    mh "Oh, look at that. Oliver's here as well. Perfect timing."
    scene d17s05b-14-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_heh1 noloop
    op "Hi Miss Harris!"
    play voice2 d1s5_mchappy noloop volume 1.6
    mc "Did you know that he was into bikes?"
    scene d17s05b-16-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_laugh noloop
    mh "He really doesn't seem like the type, does he?"
    scene d17s05b-15-sy-mh-oliver-lyssa with dissolve
    play voice2 d9s2_mcyes noloop volume 1.8
    mc "He's a man of many mysteries."
    scene d17s05b-17-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_ugu noloop
    mh "Come on in, you two."

    $ renpy.music.set_volume(0.3, 4.0, "music")
    scene d17s05b-18-sy-mh-oliver-lyssa with fade
    play voice3 lissa_thinking noloop
    mh "*Laughs* You two make yourselves comfortable."
    mh "Can I get you two something to drink?"
    scene d17s05b-19-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_thinking_emm1 noloop
    mc "I'll just have whatever he's having."
    scene d17s05b-20-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_haha noloop
    mh "*Chuckles* Alright. Oliver?"
    scene d17s05b-21-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_okay1 noloop
    op "I could go for some tea."
    scene d17s05b-22-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_ugu2 noloop
    mh "Tea it is then."
    play sound maria_kiss3
    scene d17s05b-23-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_thinking_mmm1 noloop
    pause

    scene d17s05b-24-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_arrogant_huh3 noloop
    mc "So, how's it been? Been a bit since last time."
    scene d17s05b-25-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_ehh2 noloop
    op "Yes... Last time..."
    scene d17s05b-26-sy-mh-oliver-lyssa with dissolve
    play voice2 d1s2_hmm noloop volume 1.6
    mc "...You still nervous about that?"
    scene d17s05b-27-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_hmm1 noloop
    op "I just... We... Miss Harris."
    scene d17s05b-28-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_angry_cough1 noloop
    mc "Oliver, listen to me. I told Lyssa."
    scene d17s05b-29-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_ugh noloop
    op "You what!?"
    scene d17s05b-30-sy-mh-oliver-lyssa with dissolve
    play voice3 dahlia_thinking_hmm2 noloop
    mh "Everything alright over there?"
    scene d17s05b-31-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_yes_sure1 noloop
    mc "Everything's good."
    scene d17s05b-32-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_ehh1 noloop
    op "I—I'm sorry. I didn't mean to raise my voice."
    scene d17s05b-33-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_yes_yeah4 noloop
    mc "It's alright. But yeah, I told Lyssa. And you know what happened?"
    mc "Absolutely nothing."
    mc "She's completely fine with it."
    scene d17s05b-34-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_huh2 noloop
    op "She...what?"
    scene d17s05b-35-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_thinking_hmm5 noloop
    mc "Think about it, dude. You've been working with her for a bit now, right?"
    mc "Does she really seem like the type that'd be bothered by something like that?"
    scene d17s05b-36-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_emm1 noloop
    op "I mostly only know her in a professional setting."
    scene d17s05b-37-sy-mh-oliver-lyssa with dissolve
    play voice2 d1s5b_ehhh noloop volume 1.6
    mc "...Well shit."
    mc "Anyway. The point is, she's fin—"
    scene d17s05b-38-sy-mh-oliver-lyssa with dissolve
    play voice3 dahlia_disappointed_hmm1 noloop
    mh "What are you two talking about?"
    scene d17s05b-39-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_thinking_oh1 noloop
    mc "Oh we were just reminiscing about the last party I threw."
    play sound sfx_plate_place1
    scene d17s05b-40-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_haha2 noloop
    mh "You weren't teasing the poor thing, were you?"
    scene d17s05b-41-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_no_no2 noloop
    mc "He was just nervous about the whole thing, and I explained to him that there was nothing to worry about."
    scene d17s05b-42-sy-mh-oliver-lyssa with dissolve
    play voice3 dahlia_thinking_hmm1 noloop
    mh "Olie, it's okay. {w}Look at me."
    scene d17s05b-43-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_thinking_mmm4 noloop
    mct "Shit. Is Oliver having a panic attack?"
    scene d17s05b-44-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_moan3 noloop
    mh "I'm right here. Breathe with me."
    mh "I know everything that happened."
    mh "Everything is alright. I'm not mad at you."
    scene d17s05b-45-sy-mh-oliver-lyssa with dissolve
    play voice2 d2s9_confused noloop volume 1.6
    mc "I...I'm sorry. I wouldn't have brought it all up if I had known."
    scene d17s05b-46-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_yeah1 noloop
    op "It's...alright. It was just, a lot."
    scene d17s05b-47-sy-mh-oliver-lyssa with dissolve
    play voice3 dahlia_thinking_hmm4 noloop
    mh "Hm, how about we play a game? Get our minds off of this and think of something else."
    mh "How about Cards Against Humans? See where we go from there."
    scene d17s05b-48-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_aga1 noloop
    op "I think that might be fun."
    scene d17s05b-49-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_yes_yeah2 noloop
    mc "I'm down with whatever you two want to do."
    scene d17s05b-50-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_laugh2 noloop
    mh "Cards Against Humans it is then."
    mh "So... Who went to the bathroom most recently?"
    scene d17s05b-51-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_surprised_what1 noloop
    mc "I'm sorry, what?"
    scene d17s05b-52-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_ha noloop
    mh "Have you never played Cards Against Humans?"
    scene d17s05b-53-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_thinking_mmm5 noloop
    mc "I've heard about it, but I've never played."
    scene d17s05b-54-sy-mh-oliver-lyssa with dissolve
    play voice3 dahlia_thinking_hmm3 noloop
    mh "Well, the game starts by picking the most recent person that went uh...\"potty.\""
    scene d17s05b-55-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_laugh1 noloop
    op "*Chuckles* I think it's a pretty ingenious way to randomize the starting player."
    scene d17s05b-56-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_arrogant_huh1 noloop
    mc "Huh, well, I think I might be the one that most recently took a shit then?"
    scene d17s05b-57-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_haha2 noloop
    mh "Great. We found our Card Czar."
    scene d17s05-32-sy-mh-stacy-lyssa with fade
    pause
    scene d17s05b-59-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_ugu2 noloop
    mh "Alright, {i}Czar{/i}, pick a black card."
    scene d17s05-37-sy-mh-stacy-lyssa-house-extra with dissolve
    play voice2 d1s5_mcthinks noloop
    mc "\"Men like...\""
    scene d17s05b-60-sy-mh-oliver-lyssa with dissolve
    mc "Oh this'll be fun. You two have to pick the most heinous white card, right?"
    scene d17s05b-62-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_ugu1 noloop
    op "Mm-hm."
    scene d17s05b-63-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_aga noloop
    mh "Now shuffle the white cards and pick the funniest one."
    scene d17s05b-64-sy-mh-oliver-lyssa with dissolve
    play voice2 d14s16_smell noloop
    pause
    scene d17s05b-65-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_thinking_hmm1 noloop
    mc "The cards you two picked are, \"Men like... Women with daddy Issues.\""
    mc "and, \"Men like... Shitting out a part of a turd, sucking it back in, and shitting it back out over and over so you're basically fucking yourself with your own shit...\""
    scene d17s05b-66-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_arrogant_huh3 noloop
    mc "Well, huh."
    scene d17s05b-67-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_laugh2 noloop
    mh "*Snickers*"
    scene d17s05b-69-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_arrogant_heh1 noloop
    mc "I kinda {i}have{/i} to go with that one at this point."
    play voice3 lissa_yes noloop
    mh "One point for me then."
    scene d17s05b-71-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_disgust_ooh1 noloop
    mc "Jesus, Lyssa."
    scene d17s05-46-sy-mh-stacy-lyssa with dissolve
    play voice3 dahlia_happy_hmm2 noloop
    mh "Hey, don't judge me."

    scene black
    show screen scene_transistion(_("30 minutes later"))
    with Fade(0.8, 0.5, 0.1)
    pause

    hide screen scene_transistion
    scene d17s05b-72-sy-mh-oliver-lyssa
    with Fade(0.1,0.5,0.8)
    play voice4 oliver_ehh2 noloop
    op "That's messed up, [mcname]."
    scene d17s05b-73-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_happy_yay2 noloop
    mc "I'm playing the game the way it was meant to be played!"
    scene d17s05-51-sy-mh-stacy-lyssa with dissolve
    play voice3 lissa_shyoh noloop
    mh "The poor orphans."
    scene d17s05b-75-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_happy_a1 noloop
    mc "This is an organized attack on my character."
    play sound sfx_doorbell_lyssa
    scene d17s05-53-sy-mh-stacy-lyssa with dissolve
    play voice3 dahlia_thinking_oh noloop
    mh "Oh, I think that's our food."
    scene d17s05b-77-sy-mh-oliver-lyssa with dissolve
    mh "I'll go get it."
    scene d17s05b-78-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_heh2 noloop
    op "This has been fun."
    scene d17s05b-79-sy-mh-oliver-lyssa with dissolve
    play voice2 d9s2_yeah noloop volume 3.0
    mc "Yeah. You're good at this."
    scene d17s05b-80-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_um1 noloop
    op "I got lucky I think. I got some good cards."
    scene d17s05b-81-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_yes_yes1 noloop
    mc "Right."
    mc "Look, Oliver, I really feel bad about what happened earlier."
    scene d17s05b-82-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_ehh1 noloop
    op "Oh, it's okay. It's just...something that happens sometimes. Crappy parents do that to ya."
    op "I'm sorry if I scared you."
    scene d17s05b-83-sy-mh-oliver-lyssa with dissolve
    play voice2 d9s2_mmno noloop volume 2.5
    mc "There's nothing to be sorry about."
    mc "I shouldn't have pushed that on you outta nowhere like that."
    scene d17s05b-84-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_hmm1 noloop
    op "I...I appreciate you saying that."
    scene d17s05b-85-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_thinking_hmm4 noloop
    mc "Hm. {w}\"Crappy parents\"?"
    scene d17s05b-86-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_laugh2 noloop
    op "*Laughs* I knew I shouldn't have said that."
    scene d17s05b-87-sy-mh-oliver-lyssa with dissolve
    play voice2 d4s4_mclaugh noloop
    mc "*Chuckles* It's alright if you don't want to talk about it."
    scene d17s05b-88-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_no2 noloop
    op "No, it's okay. We came here to get to know each other, right?"
    scene d17s05b-87-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_yes_yeah1 noloop
    mc "Right."
    scene d17s05b-86-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_huh6 noloop
    op "Uhm, well, where do I begin?"
    scene d17s05b-85-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_disappointed_off1 noloop
    mc "Already not a great sign."

    scene d17s05b-89-sy-mh-oliver-lyssa with dissolve
    play voice3 daisy_hey noloop
    mh "I'm back."
    scene d17s05b-90-sy-mh-oliver-lyssa with dissolve
    mh "What were you two gossiping about without me?"
    scene d17s05-72-sy-mh-stacy-lyssa with dissolve
    play voice2 mc_arrogant_hm1 noloop
    mc "Oliver was telling me about his so called \"crappy family.\""
    scene d17s05-71-sy-mh-stacy-lyssa with dissolve
    play voice3 dahlia_angry_oh noloop
    mh "Oh, {i}oh{/i}. Those people."
    scene d17s05b-93-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_laugh1 noloop
    op "*Laughs* It's funny to me how much Miss Harris hates my family."
    scene d17s05-73-sy-mh-stacy-lyssa with dissolve
    play voice3 dahlia_disappointed_hmm2 noloop
    mh "Olie, we're off work. I told you. Call me Lyssa."
    mh "And yes, I absolutely despise your family. I hope they all burn in hell."
    scene d17s05-81-sy-mh-stacy-lyssa-house with dissolve
    play voice2 mc_arrogant_huh2 noloop
    mc "What the fuck did they do?"
    scene d17s05b-96-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_huh4 noloop
    op "Uhm, they were very...{i}traditional{/i}."
    scene d17s05-75-sy-mh-stacy-lyssa-house with dissolve
    play voice3 dahlia_disappointed_hmm1 noloop
    mh "That's an incredibly nice way of putting it."
    scene d17s05b-98-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_emm2 noloop
    op "My father was a bit of a... Uhm, domineering parent."
    op "And my mother was very submissive."
    scene d17s05b-99-sy-mh-oliver-lyssa with dissolve
    op "Yeah..."
    mct "He must've had a rough childhood."
    scene d17s05b-100-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_oh1 noloop
    op "I have an older brother as well, Sloan."
    scene d17s05b-101-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_thinking noloop
    mh "How's he doing these days?"
    scene d17s05b-102-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_yeah1 noloop
    op "He's doing well, I think? He's a bit of a hermit."
    op "I think he's in Argentina right now.{w} Said he was helping build a school for poor children?"
    scene d17s05-77-sy-mh-stacy-lyssa-house with dissolve
    play voice3 dahlia_thinking_hmm1 noloop
    mh "Sloan's the only person in Oliver's family that I can tolerate."
    mh "He's like a golden retriever. No brain cells, but he's sweet."
    scene d17s05b-104-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_laugh2 noloop
    op "*Laughs* Remember that time he sucker-punched that transphobe unconscious?"
    scene d17s05-80-sy-mh-stacy-lyssa-house with dissolve
    play voice3 lissa_haha2 noloop
    mh "*Laughs* And then he gently laid him down in the alley and drew a penis on his forehead."
    scene d17s05b-106-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_aga1 noloop
    op "He's the definition of chaotic good."
    scene d17s05b-107-sy-mh-oliver-lyssa with dissolve
    play voice2 d1s1_mmm noloop
    mct "Hm. Lyssa and Oliver are really comfortable with each other when they get going."
    mct "I suppose it makes sense. You couldn't exactly work well together if you barely knew each other."
    scene d17s05b-108-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_happy_a1 noloop
    mc "Sloan sounds amazing."
    scene d17s05b-109-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_yeah2 noloop
    op "He is, he's the reason I'm still here today."
    op "Anyway. Enough about my sad history.{w} Should we get back to the game?"
    scene d17s05b-110-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_aga noloop
    mh "Right. I think you're going next?"
    scene d17s05b-111-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_ugu2 noloop
    op "Yep."

    scene d17s05b-112-sy-mh-oliver-lyssa with dissolve
    op "\"How did I lose my virginity...\""

    scene d17s05b-113-sy-mh-oliver-lyssa with dissolve
    pause
    scene d17s05b-114-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_huh5 noloop
    op "Someone said, \"How did I lose my virginity... Three dicks at the same time.\""
    scene d17s05-90-sy-mh-stacy-lyssa-house with dissolve
    play voice3 lissa_laugh2 noloop
    mh "Bit of an overachiever. *Chuckles*"
    scene d17s05b-116-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_huh3 noloop
    op "And someone else said, \"How did I lose my virginity... Seething with quiet resentment\""
    scene d17s05b-118-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_ohgod noloop
    op "Oh God, that's horrible. *Laughs*"
    op "That one wins."
    scene d17s05-88-sy-mh-stacy-lyssa-house with dissolve
    play voice2 mc_happy_wooh3 noloop
    mc "Woo!"
    scene d17s05b-120-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_heh2 noloop
    op "Though I hope my first tim— Uhm."
    scene d17s05-99-sy-mh-stacy-lyssa-house with dissolve
    play voice3 lissa_oh2 noloop
    mh "Oh?"
    mh "Wait. Oliver, are you a virgin?"
    scene d17s05b-122-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_mmm1 noloop
    op "Uhm..."
    scene d17s05b-123-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_no6 noloop
    op "No?"
    scene d17s05b-124-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_disappointed_off2 noloop
    mc "That is the weakest attempt at a lie I've ever seen."
    scene d17s05b-125-sy-mh-oliver-lyssa with dissolve
    play voice3 dahlia_thinking_hmm3 noloop
    mh "It's alright if you are. It was just a bit surprising is all."
    scene d17s05b-126-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_yes_yeah5 noloop
    mc "Yeah. It's not like we're gonna bully you about being a virgin. What's this, high school?"
    scene d17s05b-127-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_heh1 noloop
    op "Thank you."
    op "I was a sheltered child, so I didn't really get to do a lot of stuff that kids usually do."
    scene d17s05b-128-sy-mh-oliver-lyssa with dissolve
    play voice3 dahlia_thinking_mmm2 noloop
    mh "Trust me. It's better to wait it out for someone you trust and feel safe with than just losing yourself in the hormones and getting into a bad situation."
    scene d17s05b-129-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_relief1 noloop
    op "That makes me feel better. Thank you."
    scene d17s05b-130-sy-mh-oliver-lyssa with dissolve
    play voice3 dahlia_yes_ugu noloop
    mh "Of course."
    mh "You two want to continue playing?"
    scene d17s05b-131-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_huh4 noloop
    op "I'd like that."
    scene d17s05b-126-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_yes_sure1 noloop
    mc "Sure."

    $ renpy.music.set_volume(0.6, 2.0, "music")
    scene d17s05b-132-sy-mh-oliver-lyssa with dissolve
    pause
    $ renpy.music.set_volume(0.8, 2.0, "music")
    scene d17s05b-133-sy-mh-oliver-lyssa with dissolve
    pause
    scene d17s05b-134-sy-mh-oliver-lyssa with dissolve
    pause
    scene d17s05b-135-sy-mh-oliver-lyssa with dissolve
    pause
    scene d17s05b-136-sy-mh-oliver-lyssa with dissolve
    pause
    scene d17s05b-137-sy-mh-oliver-lyssa with dissolve
    pause

    $ renpy.music.set_volume(0.3, 2.0, "music")
    scene d17s05b-138-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_laugh1 noloop
    op "This was really fun!"
    scene d17s05b-139-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_yes_aga1 noloop
    mc "It was. You're concerningly good at party games."
    scene d17s05b-140-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_um1 noloop
    op "I've, uhm, just had plenty of time to play them when I was little."
    scene d17s05b-141-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_mmm1 noloop
    mh "Don't believe his lies. He's amazing at games."
    mh "I don't remember the last time I won a game against him."
    scene d17s05b-142-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_huh2 noloop
    op "You win against me all the time!"
    scene d17s05b-143-sy-mh-oliver-lyssa with dissolve
    play voice3 dahlia_happy_hmm1 noloop
    mh "Win without you having to dumb down your abilities. Don't think I don't notice. You're not slick, Olie."
    play voice2 mc_happy_hah1 noloop
    play voice3 lissa_laugh2 noloop
    scene d17s05b-144-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_laugh2 noloop
    "*Laughs*"
    scene d17s05-140-sy-mh-stacy-lyssa-house with dissolve
    play voice3 dahlia_disappointed_ehh3 noloop
    mh "*Yawns* We should do this again."
    mh "I love having you two over."
    scene d17s05-141-sy-mh-stacy-lyssa-house with dissolve
    play voice2 d1s5_mchappy noloop volume 1.5
    mc "Maybe we can have a barbeque or something."
    scene d17s05b-147-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_huh6 noloop
    op "That sounds nice."
    scene d17s05-147-sy-mh-stacy-lyssa-house with dissolve
    play voice2 mc_arrogant_hm2 noloop
    mc "But that's for another day. It's getting pretty late. I should probably head off."
    scene d17s05b-149-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_mmm2 noloop
    op "I was thinking the same thing. I should get home before it gets too dark."
    scene d17s05b-150-sy-mh-oliver-lyssa with dissolve
    play voice3 dahlia_disappointed_ehh2 noloop
    mh "*Sigh* This is the part I hate about having people over."
    scene d17s05b-151-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_heh2 noloop
    op "You're such an extrovert."
    scene d17s05b-152-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_ugu2 noloop
    mh "Only to the people I care about."
    scene d17s05b-153-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_hmm1 noloop volume 1.6
    op "This was really fun...Lyssa."
    scene d17s05b-154-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_moan1 noloop
    mh "I'm glad you enjoyed it. It was really nice relaxing with you two today."
    scene d17s05b-155-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_disappointed_ah2 noloop
    mc "I'm not sure I'd call it \"relaxing\" exactly, though. I was fighting for my life out there."
    play voice3 lissa_haha2 noloop
    play voice4 oliver_laugh1 noloop
    scene d17s05b-156-sy-mh-oliver-lyssa with dissolve
    play voice2 d3s11b_mcheh noloop
    pause

    scene d17s05b-157-sy-mh-oliver-lyssa with fade
    play voice3 dahlia_thinking_hmm4 noloop
    mh "Be safe, alright?"
    scene d17s05-166-sy-mh-stacy-lyssa-house with dissolve
    play voice2 d9s2_mcyes noloop volume 2.2
    mc "Always."
    play voice2 d1s1_mmm noloop
    play sound maria_kiss1
    scene d17s05-167-sy-mh-stacy-lyssa-house with dissolve
    play voice3 lissa_mmm1 noloop
    pause
    play sound maria_kiss2
    scene d17s05-168-sy-mh-stacy-lyssa-house with dissolve
    pause


    scene d17s05b-162-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_thinking noloop
    mh "You too. Don't go speeding now."
    play sound maria_kiss1
    scene d17s05b-163-sy-mh-oliver-lyssa with dissolve
    play voice2 oliver_mmm1 noloop
    pause
    scene d17s05b-164-sy-mh-oliver-lyssa with dissolve
    pause
    play voice2 oliver_ah2 noloop
    scene d17s05b-165-sy-mh-oliver-lyssa with dissolve
    op "I— Uhm, wha— You—"
    scene d17s05b-166-sy-mh-oliver-lyssa with dissolve
    play voice2 mc_scared_oh3 noloop
    mc "Look at that, you broke the poor thing."
    scene d17s05-179-sy-mh-stacy-lyssa-house with dissolve
    play voice3 dahlia_disappointed_hmm2 noloop
    mh "You two stay safe now."
    scene d17s05-180-sy-mh-stacy-lyssa-house with dissolve
    play voice2 mc_yes_okay2 noloop
    mc "We will. You should go lay down a bit."
    play voice3 lissa_moan2 noloop
    scene d17s05b-169-sy-mh-oliver-lyssa with dissolve
    mh "I will. I love you."
    scene d17s05b-171-sy-mh-oliver-lyssa with dissolve
    menu:
        "I love you too"(hint="d17s05m0m01c01") if True:
            $ d17s05_love_mh_op = True
            play voice2 mc_yes_yeah1 noloop
            mc "I love you too."
        "Kiss her instead"(hint="d17s05m0m01c02") if True:

            play sound maria_kiss1
            scene d17s05-167-sy-mh-stacy-lyssa-house with dissolve
            pause
            scene d17s05b-171-sy-mh-oliver-lyssa with dissolve
            play voice2 mc_thinking_hmm3 noloop
            pass

    mc "Rest up well. I'll talk to you tomorrow. Goodnight."
    scene d17s05b-172-sy-mh-oliver-lyssa with dissolve
    mc "You alright, bud?"
    scene d17s05b-173-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_huh5 noloop
    op "Hm? What? Yes. Uhm, Goodnight, Miss Harris."

    scene d17s05b-174-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_ha noloop
    mh "Good night, Oliver. I'm sorry for the surprise kiss."
    scene d17s05b-175-sy-mh-oliver-lyssa with dissolve
    play voice4 oliver_no1 noloop
    op "No! No... I, uhm, I liked it."
    scene d17s05b-176-sy-mh-oliver-lyssa with dissolve
    play voice3 lissa_haha noloop
    mh "Well, I'm glad."
    scene d17s05b-177-sy-mh-oliver-lyssa with dissolve
    pause
    $ persistent.d17s05_mh_op = True

    stop music fadeout 4.0
    jump d17s06
# Decompiled by unrpyc: https://github.com/CensoredUsername/unrpyc
