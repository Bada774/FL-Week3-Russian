init python:
    ...
    def check_finished_endings():
        ...
        if number_of_finished_endings == 6:
    ...
    def unlock_everything():
        for i in scene_gallery_slots:
            unlock_gallery_slot("scene", i[0])
        for i in cg_gallery_slots:
            unlock_gallery_slot("cg", i[0])
        for i in extra_gallery_slots:
            unlock_gallery_slot("extra", i[0])
        for i in endings_list:
            unlock_ending(i[0][-2:])
    ...
    def  get_replay_warning(what, main_menu):
        ...
        if what == "scene":
            if main_menu:
                replay_warning = _("The game will replay the scene\naccording to a default playthrough")
            else:
                replay_warning = _("The game will try to replay the scene\naccording to your current playthrough")
